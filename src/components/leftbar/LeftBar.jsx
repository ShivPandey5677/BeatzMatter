      ...
      <div className="flex flex-col gap-11">
        ...
        <ul className="flex flex-col gap-6">
          {sidebarLinks.map((link) => {
            ...
            return (
              <li key={link.label} className={`leftsidebar-link group ${isActive && 'bg-dark'}`}>
                <NavLink to={link.route} className="flex gap-4 items-center p-4">
                  <img src={link.imgURL} alt={link.label} className={`group-hover:invert-black ${isActive && 'invert-black'}`} />
                  {link.label}
                </NavLink>
              </li>
            );
